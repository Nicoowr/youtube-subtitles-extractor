...
export const mergeCuesInsideAChapter =
  ...
  (cues: Cue[]): Cue | null => {
    const cuesInsideStep = cues.filter(
      (cue) => cue.start >= chapter.start && cue.start < chapter.end
    );
...
export const convertVttInputToCues =
  ({ parseVttInput }: Dependencies) =>
  (input: string, chapters?: Chapter[]): Cue[] => {
    ...
    return compact(
      chapters.map((chapter) => mergeCuesInsideAChapter(chapter)(cleanCues))
    );
  };
