...
describe("parseVttInput should transform an srt file into a consistent markdown paragraph", () => {
  it("should transform the srt file into an array of chunks with each chunk of the form timestamp + subtitle", () => {
    ...
    // WHEN
    const result = parseVttInput(vttInput);

    // THEN
    expect(result.length).toBeGreaterThan(0);
  });
  it("should throw if the input is not in the correct format (eg: without a timestamp", () => {
    // GIVEN
    const vttInput =
      "WEBVTT\n" +
      "\n" +
    ...
    // THEN
    expect(result).toThrow("Invalid cue timestamp (cue #0)");
  });
  it("should merge cues with respect to the given duration step", () => {
    ...
    // WHEN
    const result = parseVttInput(vttInput, stepDurationInSeconds);
